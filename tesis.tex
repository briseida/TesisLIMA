...
puntos $v_{1}$ y $v_{2}$ en dirección de $v_{1}$ a $v_{2}$, así que
$v_{1}v_{2}\neq v_{2}v_{1}$, pero estaremos de acuerdoo en que
$v_{1}v_{2}=-v_{2}v_{1}$. Un \textbf{2-simplejo orientado} es una
región triangular $v_{1}v_{2}v_{3}$ con dirección de $v_{1}$ a $v_{2}$
a $v_{3}$, claramente $v_{1}v_{2}v_{3}$ tiene el mismo orden que
...
$v_{1}v_{2}v_{3}v_{4}$ de cuatro vértices de un tetraedro sólido y
...
Queda pendiente figura de 1,2 y 3-simplejo ?????????\\
...
Dado que en nuestro caso solo estudiamos complejos simpliciales cuyo
conjunto de vértices es finito, la segunda condición no se aplica.

Sumamos $p$-cadenas sumando sus valores; el grupo resultante es
denotado por $C_{p}(\Delta)$ y es llamado el \textbf{grupos de
