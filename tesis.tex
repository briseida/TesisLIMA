...
\begin{definition}\textbf{Acciones lineales.}
  Sea $G$ un grupo y se tiene una acción de $G$ en un $K$-espacio
  vectorial $V$ de dimensión finita, es decir, una función
 $$*:G\times V \rightarrow V $$
que satisface los axiomas:
\begin{enumerate}
\item $e*v=v$, para todo $v\in V$ y donde $e$ es el neutro de $G$.
\item $(g_{1}g_{2})*v=g_{1}*(g_{2}*v)$ para todo $v\in V$ y
  $g_{1},g_{2}\in G$.
\item $g_{1}*(v+w)=g_{1}*v+g_{1}*w$, para $g_{1}\in G$ y $v,w \in V .$
\item $g_{1}*(\lambda v)=\lambda(g_{1}*v)$, para $\lambda \in K$, y
  $v\in V$ y $g_{1}\in G.$
\end{enumerate}
...
\end{theorem}
  ...
  $(\rho(gh))v=(gh)v=g(hv)=g((\rho h)v)=(\rho g)((\rho h)v)=(\rho
  g \circ \rho h)v$\\
 ...
 $((\rho g)(\rho g)^{-1})v=(\rho g)((\rho g)^{-1}v)=(\rho g)((\rho
 g^{-1})v)=(\rho g)(g^{-1}v)=g(g^{-1}v)=(gg^{-1})v=v$\\
 $((\rho g)^{-1}(\rho g))v=(\rho g)^{-1}((\rho g)v)=(\rho
 g^{-1})((\rho g)v)=(\rho g^{-1})(gv)= g^{-1}(gv)=(g^{-1}g)v=v$\\

Suponga ahora que se tiene un homomorfismo de grupos
$\rho:G\rightarrow GL(V)$, por demostrar que $gv:=(\rho g)v$, define un
acción de $G$ en $V$.
$g(v+w)=(\rho g)(v+w)=(rho g)v+(\rho g)w$\\
...
\begin{example}(\textbf{Representación estándar})
  La representación 
\end{example}
